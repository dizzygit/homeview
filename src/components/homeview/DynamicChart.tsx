...
const DynamicChart: FC<DynamicChartProps> = ({ data, selectedEntities, chartConfig, loading }) => {
      ...
      <Card className="flex-grow shadow-lg">
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-xl">
            <LineChartIcon className="h-6 w-6 text-primary" />
            Entity Chart
          </CardTitle> {/* Corrected closing tag here */}
        </CardHeader>
  ...
  return (
    <Card className="flex-grow shadow-lg">
      ...
      <CardContent className="h-[calc(100vh-20rem)] md:h-[calc(100vh-22rem)]">
        {/* Pass the received chartConfig to ChartContainer */}
          ...
          >
            ...
            {selectedEntities.map((entity) => (
              <Line
                ...
                type="monotone"
                // Use CSS variable generated by ChartContainer based on entity_id (dataKey)
                stroke={`var(--color-${entity.entity_id})`}
                strokeWidth={2}
                dot={true} // Changed from false to true for diagnostics
